...
#
# ╔═══════════════════════════════════════════════════════════════════╗
# ║      🚀 PSIPHON CONDUIT MANAGER v1.3 - IMPROVED                  ║
# ║                                                                   ║
...
download_iran_ips() {
    ...
    curl -sL "$IRAN_IPS_URL" -o /tmp/iran_ips.txt
    if [ $? -eq 0 ]; then
...
setup_ipset() {
    ...
    log "Added $(ipset list iran_ips | grep -c '^[0-9]') Iranian IP ranges to ipset"
}
...
apply_access_control() {
    ...
    if [ $TIME_DIFF -lt $TWELVE_HOURS ]; then
        # First 12 hours: Allow Iranian IPs unlimited access
        iptables -I INPUT -m set --match-set iran_ips src -j ACCEPT
        log "STATUS: First 12 hours - Iranian IPs have unlimited access"
    else
        # After 12 hours: Limit non-Iranian IPs to 5 minutes
        iptables -I INPUT -m set --match-set iran_ips src -j ACCEPT
        iptables -I INPUT -p tcp --dport 8080 -m conntrack --ctstate NEW -m recent --name conduit_limit --set
        iptables -I INPUT -p tcp --dport 8080 -m conntrack --ctstate NEW -m recent --name conduit_limit --update --seconds 300 --hitcount 1 ! -m set --match-set iran_ips src -j REJECT
...
TIMEREOF
        ...
        systemctl start conduit-smart-guard.service
    ...
    # Run smart guard immediately
    "$INSTALL_DIR/smart-guard.sh"
}
...
DOCKEREOF
            ...
            docker build -t conduit-local:latest "$INSTALL_DIR/" || {
                log_error "Failed to build local image"
                exit 1
            }
            
            CONDUIT_IMAGE="conduit-local:latest"
...
show_smart_guard_status() {
    ...
    echo ""
    read -p "Press Enter to continue..." -n 1 < /dev/tty
}
...
start_conduit() {
    if docker ps | grep -q conduit; then
        ...
        docker start conduit || {
            echo -e "${RED}Failed to start. Recreating container...${NC}"
            docker run -d \
                --name conduit \
                --restart unless-stopped \
                -p 8080:8080 \
                -v "$INSTALL_DIR/data:/data" \
                conduit-local:latest \
                --max-clients "$MAX_CLIENTS" \
                --bandwidth "$BANDWIDTH"
        }
        sleep 2
...
MGMTEOF
    ...
    log_success "Management script created at: conduit"
}
...
print_summary() {
    ...
    echo ""
    echo -e "${YELLOW}⚠️  Important: Share your server IP:8080 with users in Iran${NC}"
    echo ""
}
...
main() {
    ...
    log_info "Step 2/5: Starting Conduit..."
    run_conduit
    echo ""
    ...
    log_info "Step 4/5: Setting up auto-start and Smart Guard..."
    setup_autostart
    setup_smart_guard
    echo ""
